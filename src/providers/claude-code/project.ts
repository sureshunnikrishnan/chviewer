...
function pickPath(payload: Record<string, unknown>, input: Record<string, unknown>): string {
  return (
...
function mapCommand(event: StoredEvent): CommandEvent {
  const p = event.payload
  const input = asRecord(p.input)
  const command = asString(p.command) ?? asString(input.command) ?? "command"
...
export function projectClaudeCodeEvents(events: StoredEvent[]): AgentEvent[] {
